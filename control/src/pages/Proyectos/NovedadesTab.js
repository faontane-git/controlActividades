...
const NovedadesTab = ({ novedades }) => {
  const getTipoIcon = (tipo) => {
    switch(tipo) {
      case 'riesgo': return <FiAlertTriangle className="text-red-500" />;
      case 'logro': return <FiCheckCircle className="text-green-500" />;
      ...
      {novedades.length > 0 ? (
        <div className="space-y-4">
          {novedades.map(novedad => (
            <div key={novedad.id} className="bg-white shadow rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-300">
